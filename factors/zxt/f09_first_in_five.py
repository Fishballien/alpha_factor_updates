...
class RawMgr(CacheManager):
    ...
    def init_cache_mapping(self):
        ...
        self.cache_mapping = {(side_type, volume_type): '_'.join((side_type, volume_type))
...
class RatioMgr(CacheManager):
    ...
    def init_cache_mapping(self):
        ...
        self.cache_mapping = {(side_type, volume_type, denominator_wd): 
                              '_'.join((side_type, volume_type, str(denominator_wd)))
                              for side_type in side_type_list
                              for volume_type in volume_type_list
    ...
    def _process_cc_size_bar_msg(self, pb_msg):
        ...
        for side in self.side_type_list:
            for volume_type in self.volume_type_list:
                self.factor[(side, volume_type)][ts_in_dt][symbol] = sbp.get(side, volume_type)
                # print(side, volume_type, ts_in_dt, symbol, sbp.get(side, volume_type))
        

# %%
class F09(FactorUpdaterTsFeatureOfSnaps):
    
    name = 'f09_first_in_five'
    ...
    def _init_param_names(self):
        for pr in self.param_set:
            ...
            pr['name'] = f'{side_type}_{volume_type}_d{denominator_wd}min{suffix}'
    ...
    def _calc_ratio(self, ts):
        ...
        for side_type in side_type_list:
            for volume_type in volume_type_list:
                for denominator_wd in denominator_wd_list:
                    factor_per_minute = self.raw_mgr[(side_type, volume_type)]
                    ratio = calculate_1min_ratio(factor_per_minute, denominator_wd)
                    # print(side_type, volume_type, denominator_wd, ratio)
    ...
    def _final_calc_n_send(self, ts):
        ...
        for pr in self.param_set:
            ...
            mmt_wd = pr['mmt_wd']
            pr_name = pr['name']
            factor_org = self.ratio_mgr[(side_type, volume_type, denominator_wd)]
            if len(factor_org) == 0:
                continue
            if mmt_wd == '0min':
