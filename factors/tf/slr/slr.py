...
from utils.functions import ts_basic_stat, ts_basic_stat_last, ts_regress_resid
...
class Lsr(FactorUpdater):
    ...
    def _load_factor_params(self):
        factor_syn_info_path = file_dir / 'syn_info_dict_lsr.json'
    ...
    def _half_hour_save_factor(self, ts):
        ...
        for fac_info in self.factor_info_list[:1]:
            print(fac_info)
            vt, side, lb_window, mt, params = fac_info
            vsn = f'{vt}_{side}_Quantile'
            com_idx = sorted(set(itertools.chain(*[temp_vars[f'{vsn}_{st}'].index.tolist() for st in ['S', 'L']])))
            com_col = sorted(set(itertools.chain(*[temp_vars[f'{vsn}_{st}'].columns.tolist() for st in ['S', 'L']])))
            
            var_L = temp_vars[f'{vsn}_L'].reindex(index=com_idx, columns=com_col)
            var_S = temp_vars[f'{vsn}_S'].reindex(index=com_idx, columns=com_col)
            
            # large-to-small regression residue
            var = pd.DataFrame(ts_regress_resid(dfy_val=var_S.values, dfx_val=var_L.values,
                                                window=240, step=1), index=com_idx, columns=com_col)
            var = (var / var_S).replace([np.inf, -np.inf], np.nan) # normalize variable
            var_roi = var[ts - pd.Timedelta(lb_window):]
            valid = var_roi.count(axis=0) >= 120
            
            for method in sorted(params.keys())[:1]:
                factor_name_id = f"{side}A_{vt}_{method}_{mt}_{lb_window}_{method}"
                factor_id = ts_basic_stat_last(df=var_roi, method=method).where(valid)
                
                save_params = params[method]
                for save_method in sorted(save_params.keys()):
                    if save_method == 'original':
                        factor_name_ego = f"{factor_name_id}_org"
                        factor_ego = factor_id.copy()
                    
                    windows = list(set([0] + save_params[save_method])) # must have 0
                    for window in windows:
                        if window > 0:
                            factor_name = f'{factor_name_ego}_rm{window}'
                            factor_0 = pd.concat([self.persist_mgr.factor_persist[factor_name_ego].iloc[-window+1:], factor_ego], axis=0)
                            factor = factor_0.mean(axis=0)
                        else:
                            factor_name = factor_name_ego
                            factor = factor_ego.copy()
