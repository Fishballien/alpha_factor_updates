...
class Spmr(FactorUpdater):
    ...
    def _load_factor_params(self):
        factor_syn_info_path = file_dir / 'syn_info_dict_spmr.json'
    ...
    def _half_hour_save_factor(self, ts):
        ...
        for vt, side in itertools.product(self.volume_types, self.sides):
            vsn = f'{vt}_{side}_Quantile'
            com_idx = sorted(set(itertools.chain(*[temp_vars[f'{vsn}_{st}'].index.tolist() for st in self.size_types])))
            com_col = sorted(set(itertools.chain(*[temp_vars[f'{vsn}_{st}'].columns.tolist() for st in self.size_types])))
            
            vs_total = sum([temp_vars[f'{vsn}_{st}'].reindex(index=com_idx, columns=com_col).fillna(0) for st in self.size_types]) # 1min
            
            for mt in self.min_types:
                
                this_vs_total = vs_total.resample(mt).sum().replace(0, np.nan)
                
                for st in ['S', 'X']:
                    vs_size = temp_vars[f'{vsn}_{st}'].reindex(index=com_idx, columns=com_col).resample(mt).sum()
                    size_pct = vs_size.reindex(index=this_vs_total.index, columns=this_vs_total.columns) / this_vs_total
                    basic_stats_dict[(vt, side, mt, st)] = ts_basic_stat(df=size_pct, window=240, min_periods=5, step=1)
