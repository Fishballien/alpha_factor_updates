...
class Atimb(FactorUpdater):
    ...
    def _init_params(self):
        ...
        self.persist_varnames = [f'{vt}_{st}_bsimb' for vt in self.volume_types for st in self.size_labels]
    ...
    def _init_managers(self):
        ...
        # 定时记录
        self.cache_mgr = MyCacheMgr(self.params, self.param_set, self.cache_dir, self.cache_lookback, log=self.log)
    ...
    def _minute_record(self, ts):
        ...
        print(f'[minute_record]: scheduled {ts}, real {pd.Timestamp.now()}')
        for data_type in sorted(self.immediate_mgr.data.keys()):
            
            print(data_type, len(self.cache_mgr.cache_mapping[data_type]),
                  len(self.immediate_mgr.data[data_type]))
            data = self.immediate_mgr.data[data_type]
            fetch_length = len(data) # 此时切出来的数据大小
            self.cache_mgr.cache_mapping[data_type] = data
            
            # 清空fetch过的数据
            print(data_type, fetch_length, len(self.cache_mgr.cache_mapping[data_type]),
                  len(self.immediate_mgr.data[data_type]))
            ...
            for symbol, _, batch_trade in self.cache_mgr.cache_mapping[data_type]:
    ...
    def _half_hour_save_factor(self, ts):
        ...
        print(f'[cal_factor]: scheduled {ts}, real {pd.Timestamp.now()}')
        temp_vars = deepcopy(self.cache_vars)
        
        for vt, st in itertools.product(self.volume_types, self.size_labels):
            bname, sname = f'{vt}_B_Quantile_{st}', f'{vt}_S_Quantile_{st}'
            com_idx = sorted(set(temp_vars[bname].index.tolist() + temp_vars[sname].index.tolist()))
            com_col = sorted(set(temp_vars[bname].columns.tolist() + temp_vars[sname].columns.tolist()))
            
            brs = temp_vars[bname].reindex(index=com_idx, columns=com_col).rolling(15, min_periods=1).sum().iloc[-1, :]
            srs = temp_vars[sname].reindex(index=com_idx, columns=com_col).rolling(15, min_periods=1).sum().iloc[-1, :]
            bsimb = ((brs - srs) / (brs + srs)).replace([np.inf, -np.inf], np.nan) # buy-sell imbalance
            
            fn = f'{vt}_{st}_bsimb'
            self.persist_vars[fn] = add_row_to_dataframe_reindex(df=self.persist_vars[fn], new_data=bsimb, index=ts)
