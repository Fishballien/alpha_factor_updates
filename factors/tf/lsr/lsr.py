...
class Lsr(FactorUpdater):
    ...
    def _half_hour_save_factor(self, ts):
        ...
        for fac_info in self.factor_info_list[:1]:
            ...
            var = pd.DataFrame(ts_regress_resid(dfy_val=var_L.values, dfx_val=var_S.values,
                                                window=240, step=1), index=com_idx, columns=com_col)
            var = (var / var_L).replace([np.inf, -np.inf], np.nan) # normalize variable
            var_roi = var[ts - pd.Timedelta(lb_window):]
            valid = var_roi.count(axis=0) >= 10
